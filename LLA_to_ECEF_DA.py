...
def computeVelocityVector(tableIndex, oldTime, newTime, oldPosition, newPosition):
    if tableIndex == 0:
        return 0, 0, 0
    else:
        vx = computeVelocity(oldTime, newTime, oldPosition[0], newPosition[0])
        vy = computeVelocity(oldTime, newTime, oldPosition[1], newPosition[1])
        vz = computeVelocity(oldTime, newTime, oldPosition[2], newPosition[2])
        return vx, vy, vz
    
# Read and parse input file, convert LLA to ECEF x,y,z coordinates,
# and compute velocities
# Each row of data frame should contain the following when this
# function completes:
# [timestamp, lat, long, alt, x, y, z, Vx, Vy, Vz]
def performConversion(filePath):
    lla_df = pd.read_csv(filePath, names = ["Timestamp", "Latitude", "Longitude", "Altitude"], index_col = 0, nrows = 10)
    pd.options.display.float_format = '{:.5f}'.format
    
    # Convert altitude from km to m
    lla_df["Altitude"] *= 1000
    
    xCoord = []
    yCoord = []
    zCoord = []
    for index, row in lla_df.iterrows():
        x, y, z = computeECEF(row["Latitude"], row["Longitude"], row["Altitude"])
        xCoord.append(x)
        yCoord.append(y)
        zCoord.append(z)
    ...
    prevIndex = 0
    for index, row in lla_df.iterrows():
        #x, y, z = computeVelocityVector(row["Latitude"], row["Longitude"], row["Altitude"])
        #xCoord.append(x)
        #yCoord.append(y)
        #zCoord.append(z)
        prevIndex = index
